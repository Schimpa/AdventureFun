 ...
 */
public class Enemy_Alien_Special extends LivingObject {
    ...
    public Enemy_Alien_Special(MainWindow game, float x, float y, float sizeX, float sizeY, World world, Player player){
        super(game);
    ...
    public void init(float x,float y,float sizeX,float sizeY,World world){
        ...
        body.setUserData("Enemy_Alien_Special");

        bullet = new Bullet(game,-100,-100,0.8f,0.8f,world,game.getAssets().getBullet_feuerkugel(),true);
        bullet.setBulletSound(game.getAssets().getSound_shoot_laser_03());
        bullet.setSpeed(new Vector2(9,9));
        bullet.setReloadTime(2.0f);

        currentFrame = new TextureRegion();

        walkAnimation = new ObjectAnimation(game.getAssets().getAlien_special(),4,1,0,3,0.15f);
